...
from agents import AsyncOpenAI, OpenAIChatCompletionsModel   # ✅ import Gemini-compatible client
...
agent = Agent(
    name="Tax Agent",
    instructions=(
        "You are a Tax Assistant Agent for students and freelancers in Pakistan. "
        "Use the retrieve_context tool to fetch tax knowledge. "
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() == "exit":
            break
        try:
            result = Runner.run_sync(agent, user_input, run_config=config)   # ✅ run with config
            print(f"\n📘 {result.final_output}\n")
